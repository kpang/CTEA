...
## Plot overlap between 30 ASD and DD gene sets and the top 10% of genes highly expressed in each of 30 major cell types of adolescent mouse nervous system.

data_cell_type <- scan("cell_type_1.txt", sep = "\t")

## The file "gene_set_1.txt" is the same as the file "sample_id_agg_3.txt".
...
text(length(data_function_set)/2, par("usr")[3]-10.5, labels = "Top 10% of genes             expressed in individual major cell types", srt = 0, pos = 1, xpd = TRUE, cex = 1.5, font = 2)

text(length(data_function_set)/2-3.4, par("usr")[3]-10.5, labels = "highly", srt = 0, pos = 1, xpd = TRUE, cex = 1.5, col = 2, font = 2)
...
text((max(data_score[,1])+1)/2, par("usr")[3]+max(data_score[,2])+3.2, labels = "Adolescent mouse nervous system", srt = 0, pos = 1, xpd = TRUE, cex = 2, font = 2)
