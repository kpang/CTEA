...
## Plot overlap between 30 ASD and DD gene sets and the top 10% of genes highly expressed in each of 22 major cell types of embryonic mouse brain.

data_cell_type <- scan("cell_type_1.txt", sep = "\t")

## The file "gene_set_1.txt" is the file "sample_id_agg_5.txt" with small changes.
...
text(length(data_function_set)/2, par("usr")[3]-5.4, labels = "Top 10% of genes             expressed in individual major cell types", srt = 0, pos = 1, xpd = TRUE, cex = 1.5, font = 2)

text(length(data_function_set)/2-3.1, par("usr")[3]-5.4, labels = "highly", srt = 0, pos = 1, xpd = TRUE, cex = 1.5, col = 2, font = 2)

par(cex.axis = 1.5)

axis(2, length(data_disease_set):1, data_disease_set, las = 2)

text(-10.3, 26, labels = "EGS", srt = 0, pos = 1, xpd = TRUE, cex = 1.5)

lines(c(-9.3, -9.3), c(21, 30), lty = 1, lwd = 1.5, xpd = TRUE)

text(-10, 20.5, labels = "GWAS", srt = 0, pos = 1, xpd = TRUE, cex = 1.5)

text(-10.3, 15, labels = "LC", srt = 0, pos = 1, xpd = TRUE, cex = 1.5)

lines(c(-9.3, -9.3), c(10, 19), lty = 1, lwd = 1.5, xpd = TRUE)

text(-10.3, 6.5, labels = "EGS", srt = 0, pos = 1, xpd = TRUE, cex = 1.5)

lines(c(-9.3, -9.3), c(3, 9), lty = 1, lwd = 1.5, xpd = TRUE)

text(-10.3, 2, labels = "LC", srt = 0, pos = 1, xpd = TRUE, cex = 1.5)

lines(c(-9.3, -9.3), c(1, 2), lty = 1, lwd = 1.5, xpd = TRUE)

text(-12.3, 20.5, labels = "ASD", srt = 0, pos = 1, xpd = TRUE, cex = 1.5)

lines(c(-11.3, -11.3), c(10, 30), lty = 1, lwd = 1.5, xpd = TRUE)

text(-12.3, 5.5, labels = "DD", srt = 0, pos = 1, xpd = TRUE, cex = 1.5)

lines(c(-11.3, -11.3), c(1, 9), lty = 1, lwd = 1.5, xpd = TRUE)

text((max(data_score[,1])+1)/2, par("usr")[3]+max(data_score[,2])+3.2, labels = "Embryonic mouse brain", srt = 0, pos = 1, xpd = TRUE, cex = 2, font = 2)
