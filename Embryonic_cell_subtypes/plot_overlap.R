...
## Plot overlap between 30 ASD and DD gene sets and the top 10% of genes highly expressed in each subtype of embryonic neurons.
...
text(length(data_function_set)/2, par("usr")[3]-8.9, labels = "Top 10% of genes             expressed in individual cell subtypes", srt = 0, pos = 1, xpd = TRUE, cex = 1.5, font = 2)

text(length(data_function_set)/2-3, par("usr")[3]-8.9, labels = "highly", srt = 0, pos = 1, xpd = TRUE, cex = 1.5, col = 2, font = 2)
...
text((max(data_score[,1])+1)/2, par("usr")[3]+max(data_score[,2])+3.2, labels = "Neurons", srt = 0, pos = 1, xpd = TRUE, cex = 2, font = 2)
