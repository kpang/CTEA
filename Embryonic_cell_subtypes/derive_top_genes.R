## Average the counts of each gene in all cell clusters of each embryonic neuron subtype.
...
for (i in 1:length(data_cell_type))
{
    ...
    {
        data_expression <- data.frame(data_expression, data_expression[,2])
    }
...
## Rank all genes in each cell subtype.
...
## Extract the top 10% of genes with the highest expression levels in each cell subtype.
...
## Background genes for all cell subtypes.
